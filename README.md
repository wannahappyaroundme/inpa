...
>
> **2026-06-28 추가 (콘텐츠 보호·판촉물 골격·버그수정)**: **콘텐츠 보호** — 공유뷰 워터마크·복사 차단 + 전역 이미지 저장 방해. **판촉물 골격** — 명함·달력·리플렛·팜플렛·파일보관함 카테고리 + 시드. **보안 하드닝** — DoS·비용폭탄·무차별대입 방어. **버그 3종 수정** — 히트맵 분류 배지가 숫자(0/1/2) 대신 분류명(공통/생명/손해)으로, '상세정보 저장' 안내가 폼을 다시 수정하면 사라지게, 설명의무 안내문 정리.
>
