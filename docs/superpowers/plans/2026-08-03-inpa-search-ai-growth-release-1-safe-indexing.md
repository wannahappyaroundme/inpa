...
**Released:** 2026-08-03 via PR #158, merge SHA `994c871`. GitHub CI run `30825195006` passed all jobs; Vercel and Render production verification completed, including a five-minute zero-error Vercel log observation.
...
- [x] **Step 1: 공유 폴더 상태와 원격 기준을 확인한다**
...
- [x] **Step 2: 격리 브랜치와 worktree를 만든다**
...
- [x] **Step 2: targeted test가 RED인지 확인한다**
...
- [x] **Step 3: `search-policy.ts`와 루트 fail-closed 메타를 구현한다**
...
- [x] **Step 4: 공개 페이지만 명시적으로 열고 sitemap을 정렬한다**
...
- [x] **Step 5: 정책 테스트를 GREEN으로 만든다**

```bash
npm run test:run -- components/__tests__/search-policy.test.tsx components/__tests__/sitemap.test.tsx
```

- [x] **Step 6: 색인 정책만 커밋한다**
...
- [x] **Step 1: backend와 frontend 실패 테스트를 쓴다**
...
- [x] **Step 3: backend sitemap 계약을 바로잡는다**
...
- [x] **Step 4: Next sitemap을 요청별 최신 목록으로 구현한다**
...
- [x] **Step 6: sitemap fail-closed 일관성만 커밋한다**
...
- [x] **Step 1: canary가 포함된 실패 테스트를 쓴다**
...
- [x] **Step 4: Vercel 전역 분석 컴포넌트를 연결한다**
...
- [x] **Step 6: canary 테스트와 전체 FE 테스트를 통과시킨다**
...
- [x] **Step 4: copy lint와 테스트를 통과시킨다**
...
- [x] **Step 2: 로컬 브라우저로 메타와 민감 라우트를 검수한다**
...
- [x] **Step 3: `superpowers:requesting-code-review`로 독립 검토한다**

정확성, 개인정보, 검색 정책, 장애 폴백, 모바일 UX 관점에서 Critical/Important를 받고 확인된 항목을 수정한다.

- [x] **Step 4: 최신 master와 차이를 확인하고 PR을 연다**
