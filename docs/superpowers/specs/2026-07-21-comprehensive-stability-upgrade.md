...
## 0. 2026-08-18 검증 결과 (원문 아래 내용은 그대로 유지)

> 이 절은 2026-08-18 세션에서 본 설계서의 지적 12건을 실제 코드와 대조하고 테스트를 돌려 확인한 결과다. **다음 세션은 같은 검증을 반복하지 말고 이 절의 '잔존' 목록부터 이어서 보면 된다.** 아래 원문(§1 이하)은 당시 설계 기록으로 보존한다.

### 이미 해소됨 (다른 릴리스가 처리, 재확인 불필요)

| 원 지적 | 해소 근거 |
|---|---|
| 보장 기준선 단위·상품군·연령·성별 판정 | 2026-07-29 `analysis/baselines.py` 신설로 판정 권위가 한곳으로 모였고 골든 테스트로 고정됨 |
| 비교·공유·영업 문구의 사실성 | `comparison_source`/`guide_source` 표기, 중립 색상, "공유 당시" + `captured_at` KST 표기 적용 |
| 소개 카드 연락처 필수 | FE·BE가 동일한 정규식으로 검증 |
| 셀프진단 동의 선커밋 | 외부 AI 호출 전 별도 트랜잭션으로 동의 증적을 먼저 커밋 |
| 예약 설정 로드 실패 시 저장 차단 | 로드 실패 상태에서 저장 경로가 막힘 |
| 미발급 공유 토큰 404 | `LEGACY_SHARE_FALLBACK_ENABLED`로 분리, 기본 `False` |
| Render Blueprint 정합 | 2026-07-22 `4251bdb` |
| 평가 명령 hermetic 실행 | `extraction_eval.py`가 존재하지 않는 `prunable` record만 무시 |

### 2026-08-18 릴리스로 처리됨

- **예약 상태·캘린더 동기화, 업무시간 겹침, 공개 예약 기간 정합** → PR #173 (머지 `52cf912`)
- **전역 오류 경계(`error.tsx`/`global-error.tsx`/`not-found.tsx`)** → PR #173
- **베타 무제한 모드 사용량 계측** → PR #174 (머지 `4b067b4`)
...
5. WorkHour 완전중복을 막는 DB 제약이 없다. 프로덕션의 기존 중복 여부를 먼저 확인한 뒤 별도로 처리해야 한다.
6. `analysis/models.py`의 `INSURANCE_TYPE` choices에 실손(3)이 없다.

(`docs/dev/20-devops-and-deploy.md`의 Render Free 플랜 표기 잔존 건은 2026-08-18에 Starter 운영 상태로 정정 완료.)
