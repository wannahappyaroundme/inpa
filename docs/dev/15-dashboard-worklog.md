...
> dev/15 — 로그인 첫 화면(대시보드)의 데이터 모델·화면 스펙. 정본 dev/06~09(MVP 슬라이스·API 계약·화면 스펙·컴플라이언스)와 dev/10(planner_baseline) 위에 얹는 **설계사 본진의 "매일 들어올 이유"** 레이어. 본 문서는 net-new 영역(캘린더·업무기록 모델)을 계약·구조·흐름으로 정의한다. 구현 코드가 아닌 모델/화면/계약을 다룬다.
...
| type | 색 도트 (캘린더 전용 팔레트) | 생성 | 소유 출처 | 액션큐 연동 |
|---|---|---|---|---|
| `expiry` 만기 | 🟣 `--cal-expiry`(보라/남보라 — **red 아님**) | auto | `CustomerInsurance` 만기일 | 사유1 (D-30↓) |
| `birthday` 생일 | 🎂 `--cal-birthday`(분홍) | auto | `Customer` 생년월일 | 사유3 (D-7) |
| `consult` 상담 | 💬 `--accent-blue` | manual | 설계사 직접 | 당일 아젠다 |
| `task` 할일 | ✅ `--success` | manual | `Task` 1:1 | 사유 외(당일 아젠다) |

> ⚠️ **캘린더 색 드리프트 차단(FE 0번 이슈 상속)**: 스캐폴드 mock의 `expiry→bg-cnone(빨강)`은 **금지**. dev/08 절대레드라인 *"red는 §97 비교안내 전용, 히트맵·캘린더 금지"*. 만기 도트는 **데이터색(cov-\*)과 분리된 중립 카테고리 팔레트**(`--cal-expiry` = 보라 계열)로 재정의한다. **단, 액션큐 만기 도트는 `--danger` 🔴 허용**(디자이너 스펙 — 액션큐 도트만 예외, 캘린더 셀은 보라). 이 이원화를 토큰 주석에 명시. (PM/디자인 동결 — §8 G-2)
...
| 카드 | 값 출처 | 표기 |
|---|---|---|
| 내 고객 | `Customer` count | `1,204` (tabular-nums) |
| 이번 달 만기 | `CalendarEvent(expiry, 이달)` count | `12` |
| 오늘 할 일 | `Task(due=today, !done, !snoozed)` count | `5` |
| 이번 달 신규 | `Customer(created 이달)` count | `8` |
| 미열람 | `share_view` 부재 24h+ count (북극성 소비) | `3` |

**계약(절대)**:
- 모든 숫자 `Intl.NumberFormat('ko-KR')` + `tabular-nums`. 강조색 = `--accent-blue`, 라벨 = `--ink-2`.
- **카드에 충족/위험 판정 금지** — 사실 카운트만. "부족 3" 같은 판정어 컴파일 차단(블랙리스트).
- 크레딧바: `GET /credit_status/` 폴링, `remaining=null`→`∞`(0은 무제한 센티넬 — foliio §8 트랩 상속, `remaining==0` 아님).
