...
|---|---|---|---|---|
| `ocr_detect` | 증권 OCR 분석 1건 | **10건** | 200건 | AI 호출 비용이 가장 큼 |
| `ai_compare` | 갈아타기 비교안내서 생성 1건 | **5건** | 100건 | §97 Phase 1.5 이후 활성화 |
| `ai_analysis` | AI 내러티브/인사이트 생성 1건 | **10건** | 200건 | 히트맵 텍스트 요약 등 |
| `ai_message` | AI 카톡 메시지 초안 생성 1건 | **5건** | 100건 | Phase 1.5 이후 활성화 |
...
class Plan(models.Model):
    ...
    # action별 월 한도 — null = 무제한
    limit_ocr_detect   = models.PositiveIntegerField(null=True, default=10)
    limit_ai_compare   = models.PositiveIntegerField(null=True, default=5)
    limit_ai_analysis  = models.PositiveIntegerField(null=True, default=10)
    limit_ai_message   = models.PositiveIntegerField(null=True, default=5)
...
class UsageMeter(models.Model):
    ...
    """
    ACTION_CHOICES = (
        ('ocr_detect',   'OCR 증권 분석'),
        ('ai_compare',   'AI 비교안내서'),
        ('ai_analysis',  'AI 분석 내러티브'),
        ('ai_message',   'AI 카톡 메시지'),
    )
...
### 3.1 check_and_increment (핵심 유틸)

```python
# inpa/membership/limits.py

from django.db import transaction
from .models import UsageMeter, Subscription

class LimitExceeded(Exception):
    """한도 초과. API는 429로 응답, FE는 업그레이드 유도 모달."""
    def __init__(self, action: str, current: int, limit: int):
        self.action  = action
        self.current = current
        self.limit   = limit

def check_and_increment(user, action: str) -> dict:
    """
    ...
    lim  = plan.get_limit(action)   # None = 무제한

    with transaction.atomic():
        meter, _ = UsageMeter.objects.select_for_update().get_or_create(
            user=user, action=action, year_month=ym,
            defaults={'count': 0}
        )

        if lim is not None and meter.count >= lim:
            raise LimitExceeded(action=action, current=meter.count, limit=lim)

        meter.count += 1
        meter.save(update_fields=['count', 'updated_at'])

    remaining = (lim - meter.count) if lim is not None else None
    return {'action': action, 'count': meter.count, 'limit': lim, 'remaining': remaining}
...
```python
# 예: OCR detect 뷰
from inpa.membership.limits import check_and_increment, LimitExceeded

class InsuranceDetectView(APIView):
    permission_classes = [IsAuthenticated, IsOwner]

    def post(self, request, *args, **kwargs):
        # 1. 국외이전 동의 게이트 (기존 412)
        if not request.user.customer.consent_overseas_at:
            return Response({'reason': 'CONSENT_OVERSEAS_REQUIRED'}, status=412)

        # 2. 사용량 게이트 (한도 초과 시 429)
        try:
            usage = check_and_increment(request.user, 'ocr_detect')
...
FE는 이 429를 받으면 업그레이드 유도 모달을 띄운다. 에러 메시지·차단 UI는 사용하지 않는다.
...
class PlanAdmin(admin.ModelAdmin):
    list_display  = ['code', 'display_name', 'price_krw',
                     'limit_ocr_detect', 'limit_ai_compare',
                     'limit_ai_analysis', 'limit_ai_message', 'is_active']
    list_editable = ['limit_ocr_detect', 'limit_ai_compare',
                     'limit_ai_analysis', 'limit_ai_message', 'price_krw']
    # 코드 배포 없이 한도·가격 직접 수정 가능
...
| AC | 기준 | 검증 방법 |
|---|---|---|
| AC-B1 | Free 설계사 `ocr_detect` 10건 소진 → 11번째 호출 → 429 `LIMIT_EXCEEDED` | pytest + curl |
| AC-B2 | Plus 설계사 동일 호출 → 200건까지 통과 | pytest |
| AC-B3 | 429 응답에 `upgrade_url` 포함 | assert response.data |
...
| AC-B8 | `UsageMeter.check_and_increment` race condition: 동시 요청 10건 → count 정확히 10 (select_for_update) | pytest concurrent |
| AC-B9 | 베타 기간(Plus 과금 OFF) → 모든 설계사 Free 한도 내 동작 / Plus 카운터 무차감 모드 전환 가능 | 관리자 Plan 수정으로 Free 한도 일시 상향 |
...
[Phase 1 — MVP 슬라이스 이후]
  ...
  → check_and_increment 유틸 + LimitExceeded 예외
  → OCR detect 뷰에 계측 훅 삽입
  → GET /billing/usage/ 설계사 자가 조회
  → Django Admin 등록 (관리자 수동 운영 시작)
  → FE /settings/billing 사용량 현황 화면
  → 429 소프트 블록 + 업그레이드 유도 모달

[Phase 1.5 — Plus 과금 ON]
  → 업그레이드 안내 페이지 (계좌이체 수동 안내)
  → Admin 구독 수동 변경 운영 루틴 확정
  → AI 비교안내서 / AI 메시지 계측 훅 삽입 (Phase 1.5 기능 활성화 시)
...
| G4 | 베타 기간 한도 무차감 운영 방식 — Plan 한도 일시 상향 vs 별도 Beta Plan 코드 추가 | Plan 한도 상향(단순) | Sprint 1 착수 전 |
