...
| `notif_type` | expiry_soon / birthday_soon / consult_reminder / task_due / share_unread / board_comment / board_like |
...
> foliio `Notification` 재활용 + 인파 신규 type. cron `watchdog`(만기·갱신·생일 일배치) → 액션큐. `board_comment`/`board_like`는 게시판 댓글/좋아요 발생 시 글 작성자에게 주는 **인앱 알림**(이벤트 트리거, 일배치 아님).

### 9.2 ReminderRule

`owner_id` FK · `rule_type`(스케줄형 notif_type와 1:1: expiry_soon/birthday_soon/consult_reminder/task_due/share_unread) · `days_before`(0~90) · `enabled`(default True) · `email_enabled`(default False) · `updated_at`.

> `board_comment`/`board_like`는 N일 전 예약이 무의미한 즉시 이벤트 알림이므로 `ReminderRule` 대상이 아니다(스케줄형 5종만 규칙화).
...
`id` · `owner` FK(User SET_NULL — 소유 설계사) · `sample` FK(SET_NULL) · `form_response`(JSON — 키=form_fields[].key) · `status`(pending/reviewing/producing/shipping/completed/cancelled 상태머신) · `admin_note`(설계사에게도 노출) · `tracking_number` · `carrier` · `created_at`/`updated_at`. `OwnedQuerySetMixin` 적용. 관계: PromotionOrderStatusLog(1:N, related_name=status_logs).
...
`code`(unique: free|plus) · `display_name` · `price_krw` · `limit_ocr`(null=무제한) · `limit_ai_compare`(null) · `limit_analysis`(null) · `limit_promotion`(null) · `is_active` · `created_at`/`updated_at`. Plan 1 ← N Subscription.

> 한도 필드는 크레딧 kind 정본 4종(`ocr`·`ai_compare`·`analysis`·`promotion`)과 1:1 대응한다(§16). `null`은 무제한 sentinel.
...
`user` FK(User) · `action`(ocr|ai_compare|analysis|promotion) · `year_month`(YYYY-MM, 월별 lazy reset) · `count` · `updated_at` · `UNIQUE(user, action, year_month)`.

> `action` 집합 = 크레딧 kind 정본 4종(`ocr`·`ai_compare`·`analysis`·`promotion`, §16)과 동일. foliio `credit.py:_check_and_consume`의 `kind`를 이 4종으로 매핑한 것의 영속화. 베타 `FREE_TIER_UNLIMITED=True`로 전부 우회, 정식 출시 시 `False` flip. `limit=null`은 **무제한 sentinel**(`is_unlimited`로 판별, `remaining==0` 아님).
...
`id` · `recipient` · `email_type`(verification|resend-verification|password_reset|expiry_notify) · `sent_at` · `resend_message_id`(null) · `status`(sent|bounced|failed). 이메일 발송 감사 로그. 발송 인프라=Resend(`dev/20` 확정).
...
|---|---|---|---|
| `POST /api/v1/auth/register/` | ✦ | — | 약관 동의 통합 수집 → User(is_active=False) + 인증메일 |
| `GET /api/v1/auth/verify-email/?token=` | ✦ | — | is_active=True (TTL 24h) |
| `POST /api/v1/auth/resend-verification/` | ✦ | — | 인증메일 재발송(EmailLog.email_type=resend-verification) |
| `POST /api/v1/auth/login/` | ✦ | — | 비밀번호 검증 + is_dormant 복구 + Token 발급 (5회 실패→423 잠금 / 미인증→403) |
| `POST /api/v1/auth/logout/` | ✦ | — | Token 삭제 |
| `POST /api/v1/auth/password-reset/` | ✦ | — | 무조건 200 (이메일 노출 방지) / confirm 토큰 1회용(TTL 1h) |
...
체크리스트: 매칭 담보는 `std_detail_id`+`match_source` 동반 / 미매칭은 `unmatched[]`+`UnmatchedLog` 적재 / OCR 추출률 ≥ 85% / `ocr` 크레딧 차감(베타 우회).
...
foliio `credit.py:_check_and_consume(user, kind)`를 인파 크레딧 kind 정본 **4종 `ocr`·`ai_compare`·`analysis`·`promotion`**으로 매핑한다. 이 4종은 `UsageMeter.action`(§12.3) 및 `Plan.limit_*` 필드(§12.1)와 1:1 대응한다.

| 동작 | credit kind | Plan 한도 필드 | 무료(free) | Plus(plus) |
|---|---|---|---|---|
| 증권 OCR 등록 | `ocr` | `limit_ocr` | 허용(제한) | 허용 |
| 비교안내서 생성 | `ai_compare` | `limit_ai_compare` | 1건 체험 | 복수 |
| AI 분석/메시지 생성 | `analysis` | `limit_analysis` | 제한 | 허용 |
| 판촉물 주문 | `promotion` | `limit_promotion` | 제한 | 허용 |
| 히트맵 조회 | — (무차감) | — | 허용 | 허용 |

**402 응답 shape (한도 초과 = Payment Required, 업그레이드 유도):**
```json
{ "detail": "이번 달 한도(N건)를 모두 사용했어요.", "code": "credit_exhausted",
  "kind": "ai_compare", "membership": "free", "limit": 1, "used": 1 }
```
→ HTTP **402 Payment Required**. FE는 `UpgradeGuideModal`. 베타 `FREE_TIER_UNLIMITED=True` 우회. 한도 숫자는 **베타 90일 실측 후 토큰화** (전부 **(추정)** 유지).
...
| 가격 | 크레딧 4종(`ocr`/`ai_compare`/`analysis`/`promotion`) 무료 한도 숫자 | 전환율 레버 | 베타 90일 실측 후 토큰화 (추정 유지) |
