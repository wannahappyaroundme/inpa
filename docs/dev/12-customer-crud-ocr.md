...
> 문서 ID: `dev/12-customer-crud-ocr.md` · 2026-06-19 (v2 패치: 이메일/비번 인증·가시성 매트릭스·CRM 강화·planner_baseline neutral 통제점 반영)
...
| 이메일 인증 | 가입 직후 인증 이메일 발송 → 링크 클릭 → 계정 활성화(`is_verified=True`) |
| 로그인 | `POST /api/v1/auth/login/` — 이메일·비밀번호 → DRF Token 발급 |
| 비밀번호 찾기 | `POST /api/v1/auth/password/reset/` — 이메일 입력 → 재설정 토큰 링크 발송 → `POST /auth/password/reset/confirm/` |
...
[이메일/비번 입력]
   └─ POST /auth/register/
         └─ 인증 이메일 발송
               └─ GET /auth/verify/?token=<one-time-token>
                     └─ is_verified=True → 로그인 가능
...
**레드라인**: 가입 시 약관 동의(`Profile.terms_agreed_at`)는 설계사 본인 약관이다. 고객 국외이전 동의(`Customer.consent_overseas_at`)와 **절대 혼동하지 않는다** — 이 둘의 주체·시점·대상이 다르다(`dev/09 §5`).

### 1.3 인증 전 접근 차단 (`is_verified`)

이메일 인증을 완료(`is_verified=True`)하지 않은 계정은 API 게이트(`IsVerifiedUser`)가 403을 반환한다. 인증 이메일 재발송 엔드포인트(`POST /auth/resend-verification/`) 별도 제공.

| 화이트리스트(인증 우회) | 이유 |
|---|---|
| 회원가입·로그인·비밀번호 찾기 | 인증 전 동작 필요 |
| 이메일 인증 링크(`/auth/verify/`) | 인증 전 동작 필요 |
...
### 4.4 CustomerInsurance (foliio ♻ — 무변경)
...
- [ ] AC-A2 미인증(`is_verified=False`) 상태에서 보호 API 호출 → 403.
...
- [ ] AC-G4 설계사 약관 동의(`Profile.terms_agreed_at`) ≠ 고객 국외이전 동의 (`Customer.consent_overseas_at`) — 혼용 코드 0건.
...
| G-12 ★(v2 신규) | **이메일 발송 인프라** 확정 — SMTP(SES/SendGrid) 선택, 이메일 인증 토큰 TTL(추정 24h)·재발송 rate-limit | 가입 플로우 전체 차단 | 벤더 선택 + 환경변수 세팅 필요 |
...
**BE blocking 6종 (Sprint0 게이트):** ①북극성 6종 스키마 동결 ②정규화 v0 ~150행 + 보험사 enum + 골든셋 107PDF ③insights 카피규칙(공유뷰, `dev/08`) ④국외이전 동의 1탭 대표승인 ⑤chart_based_amount 100+ 시드값(히트맵, `dev/10`) ⑥**이메일 발송 인프라 확정(SES/SendGrid, v2 신규)**. 코드는 따라온다 — **잠가야 할 건 데이터·법무·계측·인프라 4종이다.**
