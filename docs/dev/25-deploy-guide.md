...
> 담보 분류 트리 등 기준 데이터는 1회만 수동 주입. Railway 서비스 → 우측 상단 **점 3개(⋮)** → **Shell**(또는 `railway run`) 에서 실행.

```bash
python manage.py seed_taxonomy
```

- **성공 신호**: 에러 없이 종료 + 담보 데이터 생성 메시지.
- (기준선 프리셋 시드는 컴플라이언스 게이트 G4-1 확정 전까지 **실행 금지** — `dev/20` §7 참고.)
