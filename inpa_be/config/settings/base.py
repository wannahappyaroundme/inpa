...
# 팀 초대 링크(#24): TimestampSigner max_age (7일)
TEAM_INVITE_TTL_DAYS = env.int('TEAM_INVITE_TTL_DAYS', default=7)
...
CONSULTATION_SHOWCASE_PILOT_ENABLED = env.bool(
    'CONSULTATION_SHOWCASE_PILOT_ENABLED', default=False)
...
# SDK socket timeouts guard individual stalled operations, not total wall time.
# The monotonic request deadline covers local validation and every provider
# stage. Work stops waiting at 105s, reserving 5s to build the HTTP response;
# Gunicorn's 120s ceiling therefore retains a 10s process-level margin.
CONSULTATION_COMPARISON_REQUEST_DEADLINE_SECONDS = env.float(
    'CONSULTATION_COMPARISON_REQUEST_DEADLINE_SECONDS', default=110.0)
...
CONSULTATION_SUMMARY_PROVIDER = env(
    'CONSULTATION_SUMMARY_PROVIDER', default='anthropic')
CONSULTATION_AI_REQUEST_TIMEOUT_SECONDS = env.float(
    'CONSULTATION_AI_REQUEST_TIMEOUT_SECONDS', default=180.0)
OPENAI_API_KEY = env('OPENAI_API_KEY', default='')
OPENAI_TRANSCRIPTION_MODEL = env('OPENAI_TRANSCRIPTION_MODEL', default='')
OPENAI_CONSULTATION_TRANSCRIPTION_MODEL = (
    env('OPENAI_CONSULTATION_TRANSCRIPTION_MODEL', default='')
...
OPENAI_CONSULTATION_SUMMARY_MODEL = (
    env('OPENAI_CONSULTATION_SUMMARY_MODEL', default='')
    or OPENAI_COMPARISON_MODEL
)
...
OPENAI_USD_KRW_RATE = env.float('OPENAI_USD_KRW_RATE', default=1400.0)
OPENAI_TRANSCRIPTION_USD_PER_MINUTE = env.float(
    'OPENAI_TRANSCRIPTION_USD_PER_MINUTE', default=0.006)
OPENAI_CONSULTATION_INPUT_USD_PER_MTOK = env.float(
    'OPENAI_CONSULTATION_INPUT_USD_PER_MTOK', default=0.0)
OPENAI_CONSULTATION_OUTPUT_USD_PER_MTOK = env.float(
    'OPENAI_CONSULTATION_OUTPUT_USD_PER_MTOK', default=0.0)
