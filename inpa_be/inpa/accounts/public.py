...
INTRO_DUPLICATE_WINDOW = timedelta(minutes=5)
INTRO_CONSENT_PURPOSE = '소개 카드 상담 신청·연락 동의'
        ...
        with transaction.atomic():
            ...
            recent_receipt = (
                ...
                .filter(
                    ...
                    purpose=INTRO_CONSENT_PURPOSE,
                    revoked_at__isnull=True,
                    agreed_at__gte=timezone.now() - INTRO_DUPLICATE_WINDOW,
                )
                .order_by('-agreed_at', '-pk')
                .first()
            )
            application_recorded = recent_receipt is None
            ...
            if not customer.is_agree_term:
                customer.is_agree_term = True
                customer.save(update_fields=['is_agree_term'])
        if application_recorded:
