...
COVERAGE_KEYWORDS = {
    ...
    # ★ 2026-07-09 세분류 개별 인식(PM): '소액암'·'갑상선암'은 표준 트리에 전용 leaf가 있어
    #   유사암 경로에서 떼어내 별도 경로로 이동(아래). §7 substring 트랩과 동형 — 이동 후
    #   유사암 자체(유사암진단급여금 등)·상피내암·제자리암 등 나머지 유사암군은 그대로 유사암.
    '진단비->암->유사암': ['유사암', '상피내암', '경계성종양', '기타피부암', '제자리암',
                          '상피내', '대장점막내암', '전립선암', '방광암'],
    # ★ 소액암(2026-07-09): 유사암 경로에서 분리. 표준 leaf '소액암진단비' 존재(§8 무관).
    #   '소액암(유사암)진단보험금'처럼 괄호로 '유사암'이 동반 표기되는 경우 두 키워드가 동시에
    #   서브스트링으로 존재 → 복합어 '소액암(유사암)'을 추가해 길이로 확실히 소액암이 이기게 함.
    '진단비->암->소액암': ['소액암(유사암)', '소액암'],
    # ★ 갑상선암(2026-07-09): 유사암 경로에서 분리. 표준 leaf '갑상선암진단비' 존재(§8 무관).
    '진단비->암->갑상선암': ['갑상선암'],
    # === 사망 ===
    ...
    # ★ 구체 사망 복합어를 명시 추가(2026-07-09): 범용 '사망보험금'(5자)/'사망보장'(4자)이
    #   구체 '질병사망'(4자)/'재해사망'(4자)보다 길어 longest-first 로 먼저 매칭되어 전부
    #   일반사망으로 오분류되던 버그(§7 substring 트랩) 수정. 데이터로 고침(로직 불변).
    '사망->질병->질병사망': ['질병사망보험금', '질병사망보장', '질병사망특약',
                           '질병사망', '질병으로인한사망'],
    '사망->상해->상해사망': ['상해사망보험금', '상해사망보장',
                           '상해사망', '상해로사망', '일반상해사망', '교통상해사망',
                           '대중교통상해사망'],
    '사망->재해->재해사망': ['재해사망보험금', '재해사망보장', '재해사망특약', '재해사망'],
    # === 상해 ===
    # ★ 2026-07-09 세분류 개별 인식(PM): '질병후유장해'(원인=질병)·'고도/80%이상후유장해'
    #   (중증도=고도)는 표준 트리에 전용 leaf가 있어 아래 별도 경로로 이동. 나머지(일반
    #   상해후유장해·재해후유장해 등)는 그대로 상해후유장애(=상해후유장해 leaf) 유지.
    ...
    # ★ 질병후유장해(2026-07-09): 상해후유장애 경로에서 분리. 표준 leaf '질병후유장해' 존재.
    #   복합어 '질병후유장해(3~100%)'는 부모 경로의 '후유장해(3~100%)'(더 긴 서브스트링)에
    #   먼저 먹히지 않도록 그보다 긴 전용 복합어를 추가한 것(§7 substring 트랩과 동형).
    '상해->질병->질병후유장해': ['질병후유장해(3~100%)', '질병후유장해', '질병80%이상후유장해'],
    # ★ 고도후유장해(2026-07-09): 상해후유장애 경로에서 '고도/80%이상' 계열만 분리(일반
    #   상해후유장해는 그대로 유지). 표준 leaf '고도후유장해' 존재. 복합어 '고도후유장해(80%이상)'
    #   는 부모의 '후유장해(80%이상)'류보다 길게 만들어 확실히 이기게 함.
    '상해->상해->고도후유장해': ['고도후유장해(80%이상)', '고도후유장해', '상해80%이상후유장해',
                              '후유장해(80%이상)'],
    ...
    '진단비->암->일반암': ['일반암', '암진단', '다발성소아암진단',
                          '소아백혈병진단', '16대특정암진단',
                          '3대질환암진단', '일반암진단'],
    # ★ 특정암(2026-07-09): 일반암 경로에서 분리. 표준 leaf '특정암진단비' 존재.
    #   ★ 트랩: '특정(소액)암진단비'처럼 괄호가 끼면 '특정암'이 연속되지 않아 매칭 안 됨
    #   (기존에도 실패하던 케이스 — 별도 노트, 이번 스코프 아님). 괄호 없는 순수 '특정암'만 대상.
    '진단비->암->특정암': ['특정암진단', '특정암'],
    # === 뇌 ===
...
# 테이블 헤더/설명 등 "이름이 아닌" 키워드 — _extract_person_name 오탐 방지용.
# (2026-07-09: core/ocr/pii_mask.py 의 신원 마스킹도 동일 오탐 방지를 위해 재사용하므로
#  모듈 레벨로 승격했다. 값/용도 변경 없음 — 기존 함수 내부 지역변수 위치만 이동.)
