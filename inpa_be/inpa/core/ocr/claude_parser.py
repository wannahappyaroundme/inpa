...
from inpa.core.ocr.ocrparsing import COVERAGE_KEYWORDS, is_keyword_excluded, strip_exclusion_parens
...
- 그 외 치료/입원/약제비 등 진단이 아니면서 수술·처치 섹션에도 정확히 해당하지 않는 담보는
  **unmatched_coverages 로** 보내세요 (case 생성 X).
  - 예: "암입원비", "조혈모세포이식수술비", "각막이식수술비", "표적항암치료비", "암직접치료입원비" → unmatched.
...
진단비 -> 암 -> 일반암
  ...
  **진단비-암 아님 (다른 섹션)**: 암수술비→'수술' 섹션 / 항암방사선·항암약물치료비→'처치' 섹션 / 암입원·암직접치료·표적항암·면역항암·조혈모세포이식수술 등→unmatched. 어느 경우든 진단비 버킷에 합산 금지(그래프 부풀림).
...
실손 의료비 -> 질병 -> 질병 입원 의료비
  키워드: 질병입원, 질병 입원, 질병으로입원, 질병입원의료비, 질병입원비, 질병입원형, 질병입원일당, 질병입원치료, 질병으로입원치료
...
실손 의료비 -> 상해 -> 상해 입원 의료비
  키워드: 상해입원, 상해 입원, 다쳐서입원, 상해입원의료비, 상해입원비, 상해입원형, 상해입원일당, 상해입원치료, 다쳐서입원치료
...
수술 -> 암 -> 암수술비
  키워드: 암수술비, 암수술급여금
  주의: 조혈모세포이식/장기이식/각막이식/흉터복원/골절/화상/인공관절/호흡기/피부 등 특수수술은 unmatched (1차 미지원).

### 처치 (항암 약물/방사선)

처치 -> 항암 -> 항암약물치료
  키워드: 항암약물치료비, 항암약물치료, 표적항암약물치료
처치 -> 항암 -> 항암방사선치료
...
def _add_coverage(ocr, cov, default_payment, default_warranty,
    ...
    # 대칭 가드: 신규 수술/처치 섹션에 '순수 진단' 담보가 새는 것을 차단.
    # 위 진단비 가드는 cat=='진단비'에만 적용되어 신규 섹션은 무방비 → Claude 오분류의
    # Python 백스톱. 원본명(raw)에 '진단'이 있고 처치성 토큰(수술/항암/방사선/약물/이식/치료)이
    # 전혀 없으면 진단 담보로 보고 미연결. 예: 원문 "암진단비"가 수술 섹션으로 와도 거부.
    if cat in ('수술', '처치') and original_name:
        _name_ns = original_name.replace(' ', '')
        _treatment_tokens = ('수술', '항암', '방사선', '약물', '이식', '치료')
        if '진단' in _name_ns and not any(t in _name_ns for t in _treatment_tokens):
            return
