...
_CATEGORY_MAP = {
    ...
    # 후유장해 세분류 (2026-07-09 개별 인식 — PM 확정, 표준 leaf 기존 존재)
    ('상해', '질병', '질병후유장해'): ('상해', '질병', '질병후유장해'),
    ('상해', '상해', '고도후유장해'): ('상해', '상해', '고도후유장해'),
    # 진단비
    ('진단비', '암', '일반암'): ('진단비', '암', '일반암'),
    ('진단비', '암', '유사암'): ('진단비', '암', '유사암'),
    # 암 세분류 (2026-07-09 개별 인식 — PM 확정, 표준 leaf 기존 존재)
    ('진단비', '암', '소액암'): ('진단비', '암', '소액암'),
    ('진단비', '암', '갑상선암'): ('진단비', '암', '갑상선암'),
    ('진단비', '암', '특정암'): ('진단비', '암', '특정암'),
    ('진단비', '뇌', '뇌혈관'): ('진단비', '뇌', '뇌혈관'),
    ('진단비', '뇌', '뇌졸중'): ('진단비', '뇌', '뇌졸중'),
    ('진단비', '뇌', '뇌출혈'): ('진단비', '뇌', '뇌출혈'),
...
### ★ 분류 우선순위 (중요!)
- "유사암" 키워드가 있으면 반드시 유사암으로 (일반암보다 우선). 단, 소액암/갑상선암/특정암은
  아래 전용 분류가 있으므로 유사암/일반암보다 그 전용 분류가 우선합니다(2026-07-09).
- "보통약관(상해사망)", "보통약관 사망" → 일반사망으로 분류 (상해사망 아님!)
- 질병후유장해/고도후유장해도 아래 전용 분류가 있으므로 상해후유장애로 뭉뚱그리지 마세요(2026-07-09).
...
상해 -> 상해 -> 상해후유장애
  키워드: 상해후유장애, 후유장해, 상해후유장해, 일반상해일반후유장해, 일반상해고도후유장해, 일반상해후유장해, 교통상해후유장해, 대중교통후유장해, 기본계약(상해후유장해), 상해(3~100%), 상해(80%이상), 후유장해(3~100%), 상해후유장해(3%이상), 상해후유장해(80%이상)
  주의: "질병후유장해"·"고도/80%이상 후유장해" 계열은 여기 아님 → 아래 전용 분류로 분류하세요.

상해 -> 질병 -> 질병후유장해
  키워드: 질병후유장해, 질병80%이상후유장해, 질병후유장해(3~100%), 질병후유장해급여(3~100%)

상해 -> 상해 -> 고도후유장해
  키워드: 고도후유장해, 상해80%이상후유장해, 후유장해(80%이상), 고도후유장해(80%이상)
  주의: 일반 "상해후유장해"(고도/80%이상 아님)는 위 상해후유장애로 분류하세요.

상해 -> 상해 -> 재해상해
  키워드: 재해상해, 교통상해, 대중교통상해

### 진단비 - 암 (유사암을 먼저 체크! 소액암/갑상선암/특정암은 전용 분류 — 2026-07-09)

진단비 -> 암 -> 유사암
  키워드: 유사암, 경계성종양, 기타피부암, 제자리암, 대장점막내암, 전립선암, 방광암
  주의: "소액암"·"갑상선암"은 여기 아님 → 아래 전용 분류로 분류하세요.

진단비 -> 암 -> 소액암
  키워드: 소액암, 소액암진단급여금, 소액암(유사암)진단보험금

진단비 -> 암 -> 갑상선암
  키워드: 갑상선암, 갑상선암진단급여금, 갑상선암진단금

진단비 -> 암 -> 일반암
  키워드 (순수 진단만): 일반암, 암진단, 다발성소아암진단, 소아백혈병진단, 16대특정암진단, 3대질환암진단, 일반암진단
  **진단비-암 아님 (다른 섹션)**: 암수술비→'수술' 섹션 / 항암방사선·항암약물치료비→'처치->항암' 섹션 / 표적항암약물·표적항암방사선치료비→'처치->표적항암' 섹션 / 특수수술(골절/화상/이식 등)→'수술->특수' 섹션 / 암입원·암직접치료·면역항암 등→unmatched. 어느 경우든 진단비 버킷에 합산 금지(그래프 부풀림).
  주의: "특정암"은 여기 아님 → 아래 전용 분류로 분류하세요.
...
def claude_parse(text_lines, is_proposal=False, normalizer=None, meta=None):
    ...
    Args:
        ...
        meta: ★ 선택 out-param(dict). 넘기면 호출 결과 메타(model/outcome/usage/
            carrier_code/matched_count/unmatched_count)를 성공·실패 관계없이 채운다 —
            호출자가 `billing.credit.log_claude_usage` 를 단일 지점에서(성공·실패 모두)
            호출할 수 있게 하는 stash(프리런치 #17). None 이면 기존 동작과 완전히 동일
            (부작용 없음 — 기존 호출부/테스트 하위호환).
    ...
    try:
        ...
        _set_meta(outcome='package_missing', usage=None)
        return None
    ...
    try:
        ...
        if ocr_data:
            ...
            # 담보 0건이면 '기술적으론 성공했지만 아무것도 못 건짐' = empty(별도 outcome).
            outcome = 'success' if cov_count > 0 else 'empty'
            _set_meta(outcome=outcome, usage=usage, carrier_code=company_code,
                      matched_count=cov_count, unmatched_count=unmatched_count)
        else:
            # 파싱은 됐으나 Ocr_Data 변환 실패(예외는 _convert_to_ocr_data 내부에서 격리·로깅됨).
            _set_meta(outcome='json_invalid', usage=usage)
        return ocr_data
        ...
        logger.warning('[claude_parser] API timeout/connection error (after SDK retries): %s',
                       type(e).__name__)
        _set_meta(outcome='timeout', usage=None)
        return None
    except Exception:
        logger.exception('[claude_parser] API error')
        _set_meta(outcome='api_error', usage=None)
        return None
