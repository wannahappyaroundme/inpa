...
from inpa.analysis.golden_eval import (
    EVALUATION_SCOPE_NOTE, GOLDEN_SET_MIN_ACCURACY, evaluate_golden_set,
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            f"=== 폴백 골든셋 자동매칭 재현율: {result['accuracy']:.4f} "
            f"({result['passed']}/{result['total']}) — 기준선 {GOLDEN_SET_MIN_ACCURACY:.4f} ==="
        )
        self.stdout.write(EVALUATION_SCOPE_NOTE)
        ...
        self.stdout.write(
            '폴백 골든셋 분기 결과: '
            f"정확 자동매핑 {result['exact_auto_mapped']}건 / "
            f"사람 확인 {result['safe_human_review']}건 / "
            f"위험 자동오매핑 {result['unsafe_auto_mapped']}건"
        )
        self.stdout.write(
            '폴백 골든셋 안전 분기율(운영 OCR 정확도 아님): '
            f"{result['safe_decision_rate']:.4f}"
        )
