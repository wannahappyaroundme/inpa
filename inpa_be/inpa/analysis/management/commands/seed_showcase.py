...
class Command(BaseCommand):
    ...
    def _load_job_catalog():
        ...
        rows_by_key = {}
        for row in rows:
            rows_by_key.setdefault((row.sctg_cd, row.name), []).append(row)

        missing = []
        duplicates = []
        catalog = {}
        for occupation in sorted({spec.occupation for spec in CUSTOMERS}):
            key = _JOB_CATALOG_KEYS.get(occupation)
            if key is None:
