...
class ContractChecklistItemSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ('id', 'label', 'is_done', 'done_at', 'order', 'created_at', 'updated_at')
        read_only_fields = ('id', 'done_at', 'created_at', 'updated_at')


class ContactLogSerializer(serializers.ModelSerializer):
    """접촉 결과 로그 — result(택1) + memo 입력, result_display는 읽기용 한글 라벨."""
...
class CustomerSerializer(_CustomerComputedMethods, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id', 'name', 'mobile_phone_number', 'birth_day', 'gender', 'job_code',
                  'memo', 'color', 'avatar_label', 'lead_source', 'is_agree_term', 'consent_overseas_at', 'sales_stage', 'status',
