...
/** 글자를 span 단위로 그려 영문/한글 혼합 폰트를 지원하는 시네마 전용 타자기. */
function SceneTypewriter({ text, active, revealAll, charMs = 70, charClass, onChar, onDone }: {
  text: string;
  active: boolean;
  revealAll: boolean;
  charMs?: number;
...
function EmphasizedDefinition({ text }: { text: string }) {
  ...
  return (
    <>
      <span>人波</span>
      <span className="cine-dim">{text.slice(2)}</span>
    </>
  );
}
    ...
    <>
      ...
      <div role="button" tabIndex={0} onClick={advance} onKeyDown={() => {}}
        ...
        <div className={`absolute inset-0 bg-[var(--brand)] transition-opacity duration-700 ${scene.bg === "blue" ? "opacity-100" : "opacity-0"}`} aria-hidden>
          {scene.bg === "blue" && (
            <div className="absolute inset-0 flex items-center justify-center opacity-15 cine-settle">
        ...
        {mode === "closing" && (
          <div className="absolute inset-0 flex items-center justify-center" aria-hidden>
            <div className="cine-iris w-[24vmax] h-[24vmax] rounded-full bg-white" />
          </div>
        )}
      </div>
    </>
