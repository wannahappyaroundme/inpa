...
test("Next 경로에서 한 번 인코딩된 서명 토큰은 안전한 원문으로 복원한다", () => {
...
test("응답을 잃은 지원은 편집된 화면값과 무관하게 첫 전체 payload를 그대로 재사용한다", () => {
  ...
  const retry = getOrCreateSubmissionAttempt(first, form, {
    ...
    consentVersion: "changed-version",
  });
...
test("최신 동의문을 다시 불러오면 이전 동의 체크를 해제한다", () => {
  const refreshed = resetConsentForRefresh({
    name: "김설계",
    phone: "01012345678",
    careerBand: "3_5",
    currentAffiliation: "현재 GA",
    region: "서울",
    contactWindow: "afternoon",
    agreed: true,
  });

  assert.equal(refreshed.agreed, false);
  assert.equal(refreshed.name, "김설계");
});
...
test("관리 링크를 새 브라우저에서 열면 합류 확인용 토큰을 복구한다", () => {
  const storage = new MemoryStorage();

  syncManageTokenAfterLoad(storage, "fresh-manage-token", false);
  assert.equal(readStoredManageToken(storage), "fresh-manage-token");

  syncManageTokenAfterLoad(storage, "fresh-manage-token", true);
  assert.equal(readStoredManageToken(storage), null);
});
...
test("관리 화면은 계정 안내와 만료 안내가 DOM에 반영되면 종료 제목으로 초점을 옮긴다", () => {
  assert.equal(shouldFocusManageTerminalHeading("account", false), true);
  assert.equal(shouldFocusManageTerminalHeading("unavailable", false), true);
  assert.equal(shouldFocusManageTerminalHeading("ready", true), true);
  assert.equal(shouldFocusManageTerminalHeading("ready", false), false);
  assert.equal(shouldFocusManageTerminalHeading("retry", false), false);
});

test("합류 화면은 만료 안내와 합류 완료가 DOM에 반영되면 종료 제목으로 초점을 옮긴다", () => {
  assert.equal(shouldFocusJoinTerminalHeading("expired", false), true);
  assert.equal(shouldFocusJoinTerminalHeading("ready", true), true);
  assert.equal(shouldFocusJoinTerminalHeading("ready", false), false);
  assert.equal(shouldFocusJoinTerminalHeading("retry", false), false);
});
...
test("지원서는 경력과 연락 시간을 포함한 필수값을 쉬운 문장으로 검증한다", () => {
  ...
  assert.deepEqual(getPublicApplicationIssue({ ...valid, phone: "" }), {
    field: "phone",
    message: "연락처를 입력해주세요.",
  });
