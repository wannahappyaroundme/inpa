...
test("한 번 만든 지원 키는 재시도에 재사용하고 명시적으로 비운 뒤에만 새로 만든다", () => {
  let generated = 0;
  const create = () => `00000000-0000-4000-8000-00000000000${++generated}`;
  let attempt: string | null = null;

  attempt = getOrCreateSubmissionKey(attempt, create);
  assert.equal(attempt, "00000000-0000-4000-8000-000000000001");
  assert.equal(getOrCreateSubmissionKey(attempt, create), attempt);
  assert.equal(generated, 1);

  attempt = null;
  assert.equal(
    getOrCreateSubmissionKey(attempt, create),
