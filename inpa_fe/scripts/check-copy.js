...
const RULES = [
  ...
  {
    name: "화법의 하드코딩 전화번호",
    re: /(?:01[016789]|02|0[3-6][1-5]|070|080)[-\s]?\d{3,4}[-\s]?\d{4}/,
    paths: COPY_LIBRARY_PATHS,
    hint: "실제 프로필 연락처와 사용자가 입력한 수신거부 안내만 변수로 넣으세요.",
  },
  { name: "검증 불가 약속", re: /부담 없이|무조건|확실한|보장됩니다/, paths: COPY_LIBRARY_PATHS, hint: "확인 가능한 사실과 다음 행동 중심으로 바꾸세요." },
  {
    name: "수동적·검증 불가 화법",
    re: /마음에 걸리는|편하실 때|천천히 생각|결정은 고객님 몫|보험료를 아낀 분도 많아요|받을 수 있는 돈|어느 쪽이 유리|예전 상품엔 약한|굳이 바꿀 필요 없어|뭘 권하려는 건 아니고|무료|객관적으로|정확히 봐|보장 공백|10분 안에|어제 인사드린|링크 전달/,
    paths: COPY_LIBRARY_PATHS,
    hint: "확인된 내용과 고객의 작은 다음 행동, 선택지를 중심으로 다시 쓰세요.",
  },
  {
    name: "교체 전제 비교 문구",
