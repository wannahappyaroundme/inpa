...
const MILESTONES: Milestone[] = [
  ...
  },
  {
    ...
    how: "프로필/설정 API에 feature flag 노출 → 고객상세 병력 탭/폼 조건부 렌더.",
  },
  {
    id: "P5c", track: "product", priority: "P1", status: "todo", owner: "개발",
    title: "셀프진단 공개링크 noindex + 비용 가드 점검",
    detail: "익명 잠재고객 자동 리드화 = 무자격 모집 정조준 + Claude API 비용 폭주 위험.",
    how: "공개 라우트 noindex 확인, 설계사 본인 인증 게이팅 검토, 기존 throttle/cap 실측 점검.",
  },
  {
    id: "P6c", track: "product", priority: "P1", status: "todo", owner: "개발",
    title: "정규화 정확도 계측 + 정규형 토큰 전처리",
    detail: "fuzzy 매칭은 이미 구현됨(보고서 오류 정정). 진짜 할 일은 정확도 측정과 표기 정규화.",
    how: "상위 5개사 실약관 샘플로 precision/recall 측정 + 괄호·갱신형·차수 토큰 제거 전처리 보강 → IR 자산화.",
  },
  {
  ...
  {
    id: "P8c", track: "product", priority: "P2", status: "todo", owner: "개발",
    title: "정직성 카피 가드 자동 차단",
    detail: "'안전/심의완료/추천' 류 금지어가 사람 검수에만 의존하면 언젠가 샌다.",
    how: "타입/디자인 레벨에서 금지 카피를 자동 차단(린트·컴포넌트 제약).",
  },
  {
    id: "P9c", track: "product", priority: "P2", status: "todo", owner: "개발",
    title: "OCR 인식 실패 시 손입력 폴백 UX",
    detail: "현직 설계사: '3번 실패하면 손입력으로 돌아간다.' 폴백 없으면 이탈.",
    how: "인식 실패 시 부분 자동채움 + 손입력 폼으로 자연스러운 폴백.",
  },
