...
## PM 06.28 세션 ② (동의수집·QA P1/P2·OCR보정·백로그 스캔)
- **마케팅·개인정보 동의 수집**(설계서/플랜=`docs/superpowers/specs|plans/2026-06-28-marketing-consent-collection*`): 기존 `ConsentLog`(append-only)+`subject`(customer_self/planner_attested) 패턴 **재사용**, 고객 테이블 새 칼럼 0개(로그에서 파생). `ConsentLog.SCOPE_PERSONAL_INFO` 추가. **토큰 다목적화** — `make_consent_token(customer, scopes=None)`·`read_consent_token`→`{pk,scopes}` dict(★구 int 토큰=overseas 하위호환, OCR 동선 무변경). **공개 `/c` 다항목 동의**(개인정보 필수·마케팅 선택, 필수 미동의→412, 토큰 밖 scope 무시=위조방지). 셀프진단 `/d` 리드에 personal_info(항상)+marketing(선택) 기록. 등록 모달=설계사 기록(planner_attested 감사메모). 시리얼라이저 `personal_info_consent`+`consents{status,subject,agreed_at}` 파생. 고객상세 동의 배지(본인/설계사 구분)+요청 링크 복사(클립보드까지). **★보안 3중**: planner_attested는 `consent_overseas_at` 게이트 못 엶(serializer read_only+ViewSet 강제)·공개링크는 customer_self만·공개응답 PII는 마스킹 이름만.
- **QA P1(병렬 세션)**: 수기 보험 등록+제안 입력 · 공유링크 생성 FE(`/s/<token>` 90일, 보장 한눈표 공유 — §97 비교안내서 아님) · 비밀번호 변경+회원 탈퇴(★구글 가입자=비번 없어 기존 탈퇴 불가→가입 이메일 확인으로 탈퇴=개인정보 삭제권, 비번 변경 시 토큰 자동 갱신=로그아웃 방지).
- **P2 UI 정리(병렬)**: data-policy 412 raw코드 제거·모바일 탭바 safe-area 패딩·프리셋 버튼 실제 비활성·**공용 설정 탭바(기준·미팅·알림·계정) 4페이지 통일**·분석 빈상태 '수기 등록' 안내. 날짜입력=브라우저 로케일 따름(값은 YYYY-MM-DD 정상 저장, 커스텀 피커 보류). 카카오/네이버=PM 결정 유지(원복).
- **OCR 정규화 오탐 보정**: `core/ocr/ocrparsing.py` `COVERAGE_KEYWORDS`(정본, 양 파이프라인 공유) 데이터 수정 — ①양성뇌종양/뇌종양→뇌출혈 **오별칭 제거**(미매칭=보수적, 가짜매칭<미매칭=§97 안전, 종양 leaf 아직 없음) ②상피내암/제자리암→**유사암 라우팅**(substring 함정: '암진단'에 빨려감 → 긴 키워드 '상피내암' 추가로 우선순위 자동보정). 회귀테스트(`_match_coverage` 실호출) 추가. 매칭=substring+긴키워드우선이라 오탐은 코드 아닌 **사전 데이터**로 잡는다.
- **남은 것(스캔 완료, 미착수)**: ⬜ **402 업그레이드 모달** — BE는 4기능(OCR·분석·비교·판촉) 모두 402(`code='credit_exhausted'`, kind/limit/used) **이미 반환**, 베타 `FREE_TIER_UNLIMITED=True`라 미발동 → **FE 안내 모달만 부재**(OCR/분석/비교는 빨간 오류배너로만 보임). 출시 전 공용 모달 1개+402 분기 배선(저난이도). ⬜ **관리자 약관/플래그 404** — `/admin/settings/policy-versions/`(GET/POST=**PolicyVersion 모델 부재**, `ConsentLog.doc_version`은 CharField일 뿐)·`/admin/settings/flags/`(PATCH 부재). admin 전용. ⬜ **배포 env 가이드 갭**(`docs/dev/25`): `FRONTEND_BASE_URL` '선택' 표기 오해(실질 필수 — 미설정 시 인증·동의·예약 링크 localhost)·컴플라이언스 플래그(`COMPARE_*`·`ANALYZE_MEDICAL_ENABLED`) 미문서화·`.env.example` DB URL 아직 MariaDB.
